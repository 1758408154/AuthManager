...
public class MyOauth2WebResponseExceptionTranslator implements WebResponseExceptionTranslator {
    ...
    public ResponseEntity<OAuth2Exception> translate(Exception e) throws Exception {
        ...
        ase = (AuthenticationException) throwableAnalyzer.getFirstThrowableOfType(AuthenticationException.class,
                causeChain);
        if (ase != null) {
            return handleOAuth2Exception(new MyOauth2Exception(e.getMessage(), e));
        }

        ase = (AccessDeniedException) throwableAnalyzer
                .getFirstThrowableOfType(AccessDeniedException.class, causeChain);
        if (ase instanceof AccessDeniedException) {
            return handleOAuth2Exception(new MyOauth2Exception(ase.getMessage(), ase));
        }

        ase = (NoSuchMethodException) throwableAnalyzer
        ...
        // 不包含上述异常则服务器内部错误
        return handleOAuth2Exception(new MyOauth2Exception(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), e));
    }
